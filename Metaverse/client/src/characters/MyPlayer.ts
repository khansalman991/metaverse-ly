  ...
  update(
    ...
    switch (this.playerBehavior) {
      case PlayerBehavior.IDLE:
          ...
          this.scene.time.addEvent({
            ...
            callback: () => {
              ...
              if (chairItem.itemDirection) {
                const shift = sittingShiftData[chairItem.itemDirection]
                this.setPosition(chairItem.x + shift[0], chairItem.y + shift[1])
                  .setDepth(chairItem.depth + shift[2])
                
                this.playContainerBody.setVelocity(0, 0)
                this.playerContainer.setPosition(chairItem.x + shift[0], chairItem.y + shift[1] - 30)
              }
        ...
        this.playContainerBody.velocity.setLength(vx !== 0 || vy !== 0 ? speed : 0)
        ...
        if (vx !== 0 || vy !== 0) {
          ...
          if (vx > 0) this.play(`${this.playerTexture}_run_right`, true)
          else if (vx < 0) this.play(`${this.playerTexture}_run_left`, true)
          else if (vy > 0) this.play(`${this.playerTexture}_run_down`, true)
          else if (vy < 0) this.play(`${this.playerTexture}_run_up`, true)
        } else {
          const parts = this.anims.currentAnim.key.split('_')
          if (parts[1] !== 'idle' && parts[1] !== 'sit') {
            parts[1] = 'idle'
...
Phaser.GameObjects.GameObjectFactory.register(
  'myPlayer',
  function (this: Phaser.GameObjects.GameObjectFactory, x: number, y: number, texture: string, id: string, frame?: string | number) {
    ...
    sprite.body.setSize(sprite.width * collisionScale[0], sprite.height * collisionScale[1])
